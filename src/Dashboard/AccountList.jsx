...
const AccountList = () => {
    ...
    const onFinish = async (value) => {
        ...
        if (!editingAccount) {
            // Thêm mới tài khoản
            try {
                const response = await api.post('/admin/account', value);
                console.log(response.data);
    ...
    const handleDelete = async (values) => {
        try {
            const response = await api.delete(`/admin/account/${values.id}`);
            console.log(response.data)
    ...
    return (
        <>
            <Button
                ...
                }}>Thêm chủ sân mới</Button>
            <Modal open={isOpen} onCancel={() => setIsOpen(false)} title={editingAccount ? 'Sửa tài khoản' : 'Thêm tài khoản chủ sân'} footer={null}>
